...
class SeqNet(nn.Module):
    def __init__(self, cfg):
        ...
        backbone, box_head = build_resnet(name="resnet50", weights=torchvision.models.ResNet50_Weights.DEFAULT)
        ...
        print("ReID Head Type:", type(reid_head))
        print("ReID Head Attributes:", reid_head)
        ...
        #self.lw_box_softmax = 1
        #self.lw_box_contrastive = 0.1
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        #losses["loss_box_softmax"] *= self.lw_box_softmax
        #losses["loss_contrastive"] *= self.lw_box_contrastive
        return losses
...
class SeqRoIHeads(RoIHeads):
    def __init__(
        ...
        **kwargs
        ...
        self.softmax_loss = SoftmaxLoss(cfg)
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        if self.training:
            ...
            loss_box_reid, feats_reid, targets_reid = self.reid_loss(box_embeddings, box_pid_labels)
            losses.update(loss_box_reid=loss_box_reid)
            #if feats_reid is not None and targets_reid is not None:
            #    loss_box_softmax = self.softmax_loss(feats_reid, targets_reid)
            #    losses.update(loss_box_softmax=loss_box_softmax)
            #box_pid_labels_cont = torch.cat(box_pid_labels, dim=0)  
             
            # print("Embedding tiducunzai:", box_embeddings.requires_grad)  
            #loss_contrastive = self.contrastive_loss(
            #box_embeddings, 
           # box_pid_labels_cont
           # )
           # losses.update(loss_contrastive=loss_contrastive)
        else:
