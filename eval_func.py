...
def eval_detection(
    ...
    for anno, det in zip(annos, gallery_dets):
        ...
        if num_det == 0:
            count_gt += num_gt
            continue"""
        det = np.array(det) if len(det) > 0 else np.empty((0, 5))
...
def eval_search_prw(
    ...
    for i in range(len(query_dataset)):
        ...
        y_true = np.asarray(y_true)
        assert count_tp <= count_gt 
        # 计算 recall_rate 时，检查 count_gt 是否为零
        # 计算 recall_rate 时，检查 count_gt 是否为零
        if count_gt == 0:
            print(f"Warning: No ground truth for query_pid {query_pid}. Skipping recall calculation.")
            # 打印调试信息，查看是否正确匹配了 query_pid
            print(f"Processing query {i}: {query_imname}, query_pid: {query_pid}")
            print(f"  Found {len(gallery_imgs)} gallery images matching query_pid {query_pid}")
            print(f"  Ground truth for query {query_imname}: {query_gts}")

            recall_rate = 0  # 或者设置为默认值，例如 0
        else:
            # recall_rate = count_tp * 1.0 / count_gt if count_gt > 0 else 0
            recall_rate = count_tp * 1.0 / count_gt 
